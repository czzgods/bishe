<template>
	<div class="main-content" :style='{"width":"100%","padding":"20px 40px","fontSize":"15px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('kechengxinxi','报名')" @click="kechengbaomingCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
							...
							<el-button class="btn8" v-if="isAuth('kechengxinxi','私聊')" type="success" @click="chatClick(scope.row)">
		...
		<kechengbaoming-cross-add-or-update v-if="kechengbaomingCrossAddOrUpdateFlag" :parent="this" ref="kechengbaomingCrossaddOrUpdate"></kechengbaoming-cross-add-or-update>
...
<script>
	...
	import kechengbaomingCrossAddOrUpdate from "../kechengbaoming/add-or-update";
	...
	export default {
		...
		data() {
			return {
				...
				kechengbaomingCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			kechengbaomingCrossAddOrUpdate,
		},
		methods: {
			imgPreView(url){
				this.previewImg = url
			...
			kechengbaomingCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.kechengbaomingCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.kechengbaomingCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.kechengbaomingCrossaddOrUpdate.init(row.id,type);
				});
			},
