	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="擅长领域" prop="shanzhanglingyu" >
					<el-input v-model="ruleForm.shanzhanglingyu" placeholder="擅长领域" clearable  :readonly="ro.shanzhanglingyu"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="擅长领域" prop="shanzhanglingyu" >
			...
			<el-form-item v-if="type!='info'"  label="教学经验" prop="jiaoxuejingyan" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
			...
			<el-form-item v-else-if="ruleForm.jiaoxuejingyan" label="教学经验" prop="jiaoxuejingyan" >
				<span class="text" v-html="ruleForm.jiaoxuejingyan"></span>
			</el-form-item>
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'){
					...
					for (var o in obj){
						...
						if(o=='jiaoxuejingyan'){
							this.ruleForm.jiaoxuejingyan = obj[o];
							this.ro.jiaoxuejingyan = true;
							continue;
						}
						if(o=='thumbsupnum'){
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.jiaoxuejingyan = this.ruleForm.jiaoxuejingyan.replace(reg,'../../../springboot798884g4/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `bianchengjiaoshi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.bianchengjiaoshiCrossAddOrUpdateFlag = false;
											this.parent.search();
