<template>
	<div class="container">
		<div class="top-content">
			...
			<div class="right-content">
				<el-button type="danger" round @click="leaveTap">结束测试</el-button>
			</div>
		</div>
		...
		<el-card class="card_view" v-if="!isEndFlag||isSubmitFlag">
			<div v-for="(item,index) in dataList" :key="index" style="width: 100%;">
				<div class="item-content">
					...
					<span class="content" v-html="item.questionname"></span>({{item.score}}分)
	...
	export default {
		...
		methods: {
			...
			saverecord(row){
				let record = {
					userid: this.user.id,
					username: this.user.xueshengzhanghao,
			...
			scoreChange(){
				let score = 0
				for(let x in this.dataList){
					score += Number(this.dataList[x].myscore) || 0
				}
				return score
			},
		}
