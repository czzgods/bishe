	...
	<div class="main-content" :style='{"width":"100%","padding":"20px 40px","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #fff","margin":"0 0px 20px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				...
				<el-row :style='{"padding":"0px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 15px 0 10px","color":"#fff","borderRadius":"20px","background":"#4dc2de","width":"auto","fontSize":"16px","height":"34px"}' type="success" @click="backClick()">返回</el-button>
				</el-row>

				<el-row class="actions" :style='{"padding":"0px","margin":"0px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('discussbianchengjiaoshi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('discussbianchengjiaoshi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"0px solid #fff","padding":"0","color":"#000","borderRadius":"10px","background":"#fff","borderWidth":"0px","width":"100%","fontSize":"14px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="edit" v-if=" isAuth('discussbianchengjiaoshi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai17" :style='{"margin":"0 0px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>

							<el-button class="view" v-if="isAuth('discussbianchengjiaoshi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
							...
							<el-button class="btn8" type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-huifu3" :style='{"margin":"0 0px","fontSize":"14px","color":"#939393","display":"none","height":"40px"}'></span>
								回复
							</el-button>
	...
	export default {
		...
		methods: {
			...
			getDataList() {
				...
				this.$http({
					url: "discussbianchengjiaoshi/page",
					method: "get",
					params: params
				}).then(({ data }) => {
					if (data && data.code === 0) {
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussdiscussbianchengjiaoshi',query:{refid:id}});
			},
			backClick(){
				history.back()
			},
			discussistopChange(e,row){
				this.$http({
					url:'discussbianchengjiaoshi/update',
					method:'post',
					data:row
				}).then(res=>{})
			},
			...
			async deleteHandler(id ,refid ) {
				...
				await this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
					...
					await this.$http({
						url: "discussbianchengjiaoshi/delete",
						method: "post",
						data: ids
					}).then(async ({ data }) => {
						if (data && data.code === 0) {
							this.$http({
								url: 'bianchengjiaoshi/info/' + refid,
								method: "get",
							}).then(res=>{
								if(res.data&&res.data.code==0){
									res.data.data.discussnum = res.data.data.discussnum - Number(ids.length)
									this.$http({
										url: 'bianchengjiaoshi/update',
										method: "post",
										data:res.data.data
									}).then(res1=>{
