<template>
	<div class="home-content">
		...
		<div class="statis-box">
			<div id="statis1" class="statis1 animate__animated" v-if="isAuth('kechengbaoming','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-zhangjie8"></span>
				</div>
				<div class="right">
					<div class="num">{{kechengbaomingCount}}</div>
					<div class="name">课程报名总数</div>
				</div>
			</div>
		</div>
		<!-- statis -->
		...
		<!-- 2 -->
		<div class="type2">
			<div id="kechengbaomingChart1" class="echarts1 animate__animated" v-if="isAuth('kechengbaoming','首页统计')"></div>
			<div id="kechengbaomingChart2" class="echarts2 animate__animated" v-if="isAuth('kechengbaoming','首页统计')"></div>
		</div>
	</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getkechengbaomingCount();
		if(this.isAuth('kechengbaoming','首页统计')){
			this.kechengbaomingChat1();
		}
		if(this.isAuth('kechengbaoming','首页统计')){
			this.kechengbaomingChat2();
		}
	...
	methods:{
		handleScroll() {
			let arr = [
				...
				{id:'kechengbaomingChart1',css:'animate__'},
				{id:'kechengbaomingChart2',css:'animate__'},
			]
		...
		getkechengbaomingCount() {
			this.$http({
				url: `kechengbaoming/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.kechengbaomingCount = data.data
				}
			})
		},
// 1234 竖
		kechengbaomingChat1() {
			this.$nextTick(()=>{
				...
				var kechengbaomingChart1 = echarts.init(document.getElementById("kechengbaomingChart1"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						kechengbaomingChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							kechengbaomingChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		kechengbaomingChat2() {
			this.$nextTick(()=>{
				...
				var kechengbaomingChart2 = echarts.init(document.getElementById("kechengbaomingChart2"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						kechengbaomingChart2.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							kechengbaomingChart2.resize();
						};
					}else{
						this.$message({
							message: data.msg,
