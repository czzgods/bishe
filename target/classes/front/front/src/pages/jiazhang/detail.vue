	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'-'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/jiazhang?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('jiazhang','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('jiazhang','删除')" @click="delClick">删除</el-button>
						<el-button class="chatBtn" v-if="btnAuth('jiazhang','私聊')&&detail.id!=mid" @click="chatClick">私聊</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						this.title = this.detail.jiazhangxingming;
						...
						if(res.data.data.jiazhangzhanghao){
							this.fname = res.data.data.jiazhangzhanghao
						}
			...
			backClick(){
				...
				this.$router.push({path: '/index/jiazhang', query: params});
			},
			// 下载
			download(file ){
				if(!file) {
			...
			// 修改
			editClick(){
				this.$router.push(`/index/jiazhangAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此家长？') .then(_ => {
					this.$http.post('jiazhang/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
