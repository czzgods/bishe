		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">所需基础</div>
						<div class="text "  >{{detail.suoxujichu}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.kechengjianjie}}</div>
					</div>
					<div class="btn_box">
						...
						<el-button class="reserveBtn" 
							v-if="btnAuth('kechengxinxi','报名')" 
							@click="onAcross('kechengbaoming','','','','')" type="warning">
							报名
						</el-button>
					</div>
				</div>
	...
	export default {
		...
		methods: {
			...
			chatClick() {
				...
				if(this.detail.jiaoshigonghao == user.jiaoshigonghao){
					this.$message.error('不能给自己发信息')
					return false
				}
				this.$http.get('bianchengjiaoshi/query', {
					params: {
						jiaoshigonghao: this.detail.jiaoshigonghao
					}
				}).then(res => {
					if (res.data && res.data.code == 0) {
						this.fid = res.data.data.id
						this.initWebSocket(this.fid)
						this.fname = res.data.data.jiaoshigonghao
						if (res.data.data.touxiang) {
							this.fpic = res.data.data.touxiang.split(',')[0]
						} else if (res.data.data.headportrait) {
							this.fpic = res.data.data.headportrait.split(',')[0]
