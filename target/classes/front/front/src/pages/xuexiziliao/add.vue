<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="附件" prop="fujian">
				<file-upload
					tip="点击上传附件"
					action="file/upload"
					:limit="1"
			...
			<el-form-item class="add-item" label="正文" prop="zhengwen">
				<editor 
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`xuexiziliao/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
