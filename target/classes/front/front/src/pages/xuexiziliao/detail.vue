	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'-'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/xuexiziliao?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<el-button class="uploadBtn" @click="download(detail.fujian )">点击下载</el-button>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('xuexiziliao','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('xuexiziliao','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
			...
			<video v-if="detail.shipin" ref="shipin" :src="baseUrl +  detail.shipin"  controls="controls" >
				您的浏览器不支持视频播放
			</video>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card">
				<el-tab-pane label="正文" name="first">
					<div v-html="detail.zhengwen"></div>
				</el-tab-pane>
	...
	export default {
		...
		methods: {
			...
			storeup(type) {
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'xuexiziliao', userid: localStorage.getItem('frontUserid')}}).then(res => {
			...
			getThumbsupOrCrazilyStatus() {
				if(localStorage.getItem("frontToken")) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 21, refid: this.detail.id, tablename: 'xuexiziliao', userid: localStorage.getItem('frontUserid')}}).then(res => {
						if (res.data.code == 0 && res.data.data.list.length > 0) {
							this.isThumbsupnum = true;
							this.thumbsupOrCrazilyInfo = res.data.data.list[0];
						}
					});
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 22, refid: this.detail.id, tablename: 'xuexiziliao', userid: localStorage.getItem('frontUserid')}}).then(res => {
						if (res.data.code == 0 && res.data.data.list.length > 0) {
			...
			backClick(){
				...
				this.$router.push({path: '/index/xuexiziliao', query: params});
			},
			// 下载
			download(file ){
				if(!file) {
			...
			getDiscussList(page) {
				this.$http.get('discussxuexiziliao/list', {params: {page, limit: this.pageSize, refid: this.detail.id,sort: 'istop',order: 'desc'}}).then(res => {
					if (res.data.code == 0) {
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discussxuexiziliao/update',row).then(rs=>{
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						this.$http.post('discussxuexiziliao/add', this.form).then(rs2 => {
							if (rs2.data.code == 0) {
								this.form.content = '';
			...
			// 修改
			editClick(){
				this.$router.push(`/index/xuexiziliaoAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
