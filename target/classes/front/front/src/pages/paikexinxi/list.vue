		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-form-item class="list-item">
					...
					<el-select v-model="formSearch.jiaoshigonghao" placeholder="请选择教师工号" :clearable="true">
						<el-option v-for="(item, index) in jiaoshigonghaoOptions" :key="index" :label="item" :value="item"></el-option>
					</el-select>
				</el-form-item>
				...
				<el-button class="list-add-btn" v-if="btnAuth('paikexinxi','新增')" type="primary" @click="add('/index/paikexinxiAdd')">
...
<script>
	export default {
		...
		async created() {
			...
			await this.$http.get('option/bianchengjiaoshi/jiaoshigonghao').then(res => {
				if (res.data.code == 0) {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				this.$http.get(`paikexinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/paikexinxiDetail', query: params});
			},
