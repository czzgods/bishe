	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'-'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/kechengbaoming?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
			...
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.kechengmingcheng}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">课程费用</div>
						<div class="text "  >{{detail.kechengfeiyong}}</div>
					</div>
					<div class="item">
						<div class="lable">课时</div>
						<div class="text "  >{{detail.keshi}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.xueshengxingming}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('kechengbaoming','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('kechengbaoming','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('kechengbaoming','学习进展')" 
							@click="onAcross('xuexijinzhan','','','','')" type="warning">
							学习进展
						</el-button>
						<el-button class="payBtn" v-if="centerType&&(detail.ispay=='未支付'||!detail.ispay)&&btnAuth('kechengbaoming','支付')" type="warning" size="small" @click="payClick">支付</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				localStorage.setItem('crossTable',`kechengbaoming`);
			...
			payClick(){
				localStorage.setItem('paytable','kechengbaoming')
			...
			backClick(){
				...
				this.$router.push({path: '/index/kechengbaoming', query: params});
			},
			// 下载
			download(file ){
				if(!file) {
			...
			// 修改
			editClick(){
				this.$router.push(`/index/kechengbaomingAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此课程报名？') .then(_ => {
					this.$http.post('kechengbaoming/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
