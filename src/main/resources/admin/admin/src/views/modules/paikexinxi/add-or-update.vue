	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="排课时间" prop="paikeshijian" >
					<el-input v-model="ruleForm.paikeshijian" placeholder="排课时间" clearable  :readonly="ro.paikeshijian"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="排课时间" prop="paikeshijian" >
					<el-input v-model="ruleForm.paikeshijian" placeholder="排课时间" readonly></el-input>
				</el-form-item>
			</template>
			<el-form-item class="textarea" v-if="type!='info'" label="学生名单" prop="xueshengmingdan" >
				<el-input
					style="min-width: 200px; max-width: 600px;"
					type="textarea"
					:rows="8"
					placeholder="学生名单"
					v-model="ruleForm.xueshengmingdan" >
				</el-input>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.xueshengmingdan" label="学生名单" prop="xueshengmingdan" >
				<span class="text">{{ruleForm.xueshengmingdan}}</span>
			</el-form-item>
	...
	export default {
		...
		methods: {
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'){
					...
					for (var o in obj){
						...
						if(o=='paikeshijian'){
							this.ruleForm.paikeshijian = obj[o];
							this.ro.paikeshijian = true;
							continue;
						}
						if(o=='xueshengmingdan'){
							this.ruleForm.xueshengmingdan = obj[o];
							this.ro.xueshengmingdan = true;
							continue;
						}
					}
				}
			...
			jiaoshigonghaoChange () {
				this.$http({
					url: `follow/bianchengjiaoshi/jiaoshigonghao?columnValue=`+ this.ruleForm.jiaoshigonghao,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `paikexinxi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.paikexinxiCrossAddOrUpdateFlag = false;
											this.parent.search();
