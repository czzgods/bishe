...
<script>
	...
	export default {
		...
		methods: {
			...
			info(id) {
				this.$http({
					url: `discussbianchengjiaoshi/info/${id}`,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `discussbianchengjiaoshi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.discussbianchengjiaoshiCrossAddOrUpdateFlag = false;
											this.parent.search();
