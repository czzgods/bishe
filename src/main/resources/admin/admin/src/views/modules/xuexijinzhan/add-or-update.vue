<template>
	<div class="addEdit-block">
		...
		>
			...
			<el-form-item v-if="type!='info'"  label="学习进展" prop="xuexijinzhan" >
				<editor 
			...
			<el-form-item v-else-if="ruleForm.xuexijinzhan" label="学习进展" prop="xuexijinzhan" >
				<span class="text" v-html="ruleForm.xuexijinzhan"></span>
			</el-form-item>
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.xuexijinzhan = this.ruleForm.xuexijinzhan.replace(reg,'../../../springboot798884g4/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `xuexijinzhan/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.xuexijinzhanCrossAddOrUpdateFlag = false;
											this.parent.search();
