	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="家长账号" prop="jiazhangzhanghao">
				<el-select  @change="jiazhangzhanghaoChange" v-model="ruleForm.jiazhangzhanghao" placeholder="请选择家长账号">
					<el-option
			...
			<el-form-item class="add-item" label="家长电话" prop="jiazhangdianhua">
				<el-input v-model="ruleForm.jiazhangdianhua" 
					placeholder="家长电话" clearable :disabled=" false  ||ro.jiazhangdianhua"></el-input>
			</el-form-item>
	...
	export default {
		...
		methods: {
			...
			jiazhangzhanghaoChange () {
				this.$http.get('follow/jiazhang/jiazhangzhanghao?columnValue=' + this.ruleForm.jiazhangzhanghao, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						if(res.data.data.jiazhangxingming){
							this.ruleForm.jiazhangxingming = res.data.data.jiazhangxingming
						}
						if(res.data.data.jiazhangdianhua){
							this.ruleForm.jiazhangdianhua = res.data.data.jiazhangdianhua
						}
					}
				});
			},
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`xuesheng/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
