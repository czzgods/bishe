	...
	<div class="home-preview">
		...
		<div id="system" class="animate__animated">
			<div class="system_item">
				...
				<div class="system_content" v-html="systemIntroductionDetail.content"></div>
			...
			<div v-if="newsList.length" class="list list4 index-pv1">
				...
				<div @click="toDetail('newsDetail', newsList[1])" v-if="newsList.length>1" class="list-item3 animation-box">
					...
					<div class="time_item">
						<span class="icon iconfont icon-shijian21"></span>
						<span class="label">发布时间：</span>
						<span class="text">{{newsList[1].addtime}}</span>
					</div>
					<div class="publisher_item">
						<span class="icon iconfont icon-geren16"></span>
						<span class="label">发布人：</span>
				...
				<div @click="toDetail('newsDetail', newsList[2])" v-if="newsList.length>2" class="list-item5 animation-box">
					...
					<div class="time_item">
						<span class="icon iconfont icon-shijian21"></span>
						<span class="label">发布时间：</span>
						<span class="text">{{newsList[2].addtime}}</span>
					</div>
					<div class="publisher_item">
						<span class="icon iconfont icon-geren16"></span>
						<span class="label">发布人：</span>
		...
		<!-- 新闻资讯 -->
		<!-- 商品推荐 -->
		<div id="animate_recommendbianchengjiaoshi" class="recommend animate__animated">
			<div class="recommend_title_box">
				<span class="recommend_title">编程教师推荐</span>
				<span class="recommend_subhead">{{'bianchengjiaoshi'.toUpperCase()}} RECOMMEND</span>
			</div>
			<div v-if="bianchengjiaoshiRecommend.length" class="list list12 index-pv1 list12bianchengjiaoshi">
				<div class="list-body">
					<div class="item list-item animation-box" v-for="item,index in bianchengjiaoshiRecommend" :key="index" @click="toDetail('bianchengjiaoshiDetail', item)">
						<img :name="item.id" v-if="preHttp(item.touxiang)" :src="item.touxiang.split(',')[0]" alt="" />
						<img :name="item.id" v-else :src="baseUrl + (item.touxiang?item.touxiang.split(',')[0]:'')" alt="" />
						<div class="name">{{item.jiaoshixingming}}</div>
						<div class="name">{{item.shanzhanglingyu}}</div>
					</div>
				</div>
			</div>
			<div class="moreBtn" @click="moreBtn('bianchengjiaoshi')">
				<span class="text">查看更多</span>
				<i class="icon iconfont icon-gengduo1"></i>
			</div>
		</div>
		...
		<div id="animate_recommendxuexiziliao" class="recommend animate__animated">
			...
			<div v-if="xuexiziliaoRecommend.length" class="list list12 index-pv1 list12xuexiziliao">
				<div class="list-body">
					<div class="item list-item animation-box" v-for="item,index in xuexiziliaoRecommend" :key="index" @click="toDetail('xuexiziliaoDetail', item)">
	...
	export default {
		...
		methods: {
			...
			kechengxinxiRecommendCate(){
				...
				this.recommendColumn12kechengxinxi = 'kechengfenlei'
			},
			...
			handleScroll() {
				let arr = [
					...
					{id:'animate_recommendbianchengjiaoshi',css:'animate__backInRight'},
					{id:'animate_recommendkechengxinxi',css:'animate__backInRight'},
					{id:'animate_recommendxuexiziliao',css:'animate__backInRight'},
					{id:'animate_newsnews',css:'animate__backInLeft'},
				]
			...
			getList() {
				let autoSortUrl = "";
				let data = {}
				autoSortUrl = "bianchengjiaoshi/autoSort";
				if(localStorage.getItem('frontToken')) {
					autoSortUrl = "bianchengjiaoshi/autoSort2";
				}
				data = {
					page: 1,
					limit: 8,
				...
				this.$http.get(autoSortUrl, {params: data}).then(res => {
					if (res.data.code == 0) {
						this.bianchengjiaoshiRecommend = res.data.data.list;
					}
				});
				...
				if(this.recommendIndex12kechengxinxi != 0){
					data.kechengfenlei = this.recommendList12kechengxinxi[this.recommendIndex12kechengxinxi - 1].kechengfenlei
				}
				this.$http.get(autoSortUrl, {params: data}).then(res => {
					if (res.data.code == 0) {
	...
	.home-preview {
		...
		#about {
			...
			.about_item {
				...
				.about_idea2 {
					background: linear-gradient( 267deg, #F3FEFF 0%, rgba(242,254,254,0.9795) 38%, rgba(216,216,216,0) 100%);
					display: none;
					width: 285px;
					position: absolute;
					right: 16%;
		...
		#system {
			...
			.system_item {
				...
				.system_idea1 {
					top: 0;
					left: 0;
					background: linear-gradient( 267deg, #F3FEFF 0%, rgba(242,254,254,0.6) 1%, rgba(216,216,216,0) 100%);
					width: 45%;
					position: absolute;
					height: 100%;
				}
