	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'-'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/xuexijinzhan?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.tijiaoshijian}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('xuexijinzhan','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('xuexijinzhan','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card">
				<el-tab-pane label="学习进展" name="first">
					<div v-html="detail.xuexijinzhan"></div>
				</el-tab-pane>
			</el-tabs>
	...
	export default {
		...
		methods: {
			...
			backClick(){
				...
				this.$router.push({path: '/index/xuexijinzhan', query: params});
			},
			// 下载
			download(file ){
				if(!file) {
			...
			// 修改
			editClick(){
				this.$router.push(`/index/xuexijinzhanAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此学习进展？') .then(_ => {
					this.$http.post('xuexijinzhan/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
