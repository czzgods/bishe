		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-form-item class="list-item">
					...
					<el-input v-model="formSearch.kechengfenlei" placeholder="课程分类" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
				...
				<el-button class="list-add-btn" v-if="btnAuth('kechengbaoming','新增')" type="primary" @click="add('/index/kechengbaomingAdd')">
				...
				<div class="list1 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
						...
						<div class="publisher_item">
							...
							<span class="text">{{item.jiazhangzhanghao}}</span>
						</div>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				this.$http.get(`kechengbaoming/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/kechengbaomingDetail', query: params});
			},
